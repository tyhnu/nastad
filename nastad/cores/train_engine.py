...
def train_one_epoch_supernet(
    ...
    for iter_idx, data_dict in enumerate(train_loader):
        ...
        # forward pass
        with torch.cuda.amp.autocast(dtype=torch.float16, enabled=use_amp):
            losses, arch_choice = model(**data_dict, return_loss=True)
        ...
        if ((iter_idx != 0) and (iter_idx % logging_interval) == 0) or ((iter_idx + 1) == num_iters):
            ...
            block5 = "mem={:.0f}MB".format(torch.cuda.max_memory_allocated() / 1024.0 / 1024.0)
            logger.info("  ".join([block1, block2, "  ".join(block3), block4, block5]))
            logger.info("arch_chioce{}".format(arch_choice))

def train_one_epoch_supernet_ib(
    train_loader,
    model,
    optimizer,
    scheduler,
    ...
    for iter_idx, data_dict in enumerate(train_loader):
        ...
        # forward pass
        with torch.cuda.amp.autocast(dtype=torch.float16, enabled=use_amp):
            losses, arch_choice, mi = model(**data_dict, return_loss=True)
        ...
        for key, value in mi.items():
            # print(key)
            if key not in mi_tracker:
                mi_tracker[key] = AverageMeter()
            mi_tracker[key].update(value.item())
        ...
        if ((iter_idx != 0) and (iter_idx % logging_interval) == 0) or ((iter_idx + 1) == num_iters):
            ...
            block2 = "Loss={:.4f}".format(losses_tracker["cost"].avg)
            block3 = ["{:s}={:.4f}".format(key, value.avg) for key, value in losses_tracker.items() if key != "cost"]
            block6 = ["{:s}={:.4f}".format(key, value.avg) for key, value in mi_tracker.items() if key != "mi"]

            block4 = "lr_det={:.1e}".format(curr_det_lr)
            if curr_backbone_lr is not None:
                block4 = "lr_backbone={:.1e}".format(curr_backbone_lr) + "  " + block4
            block5 = "mem={:.0f}MB".format(torch.cuda.max_memory_allocated() / 1024.0 / 1024.0)
            logger.info("  ".join([block1, block2, "  ".join(block3), "  ".join(block6), block4, block5]))
            logger.info("arch_chioce{}".format(arch_choice))
            logger.info("mi={:.4f}".format(mi_tracker["mi"].avg))




def val_one_epoch_supernet(
    val_loader,
    model,
    logger,
    rank,
    ...
    for arch in range(10):
        choice=np.random.randint(2, size=7)
        for data_dict in tqdm.tqdm(val_loader, disable=(rank != 0)):
            with torch.cuda.amp.autocast(dtype=torch.float16, enabled=use_amp):
                with torch.no_grad():
                    losses = model(**data_dict, choice=choice, return_loss=True)
