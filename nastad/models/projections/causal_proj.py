...
class MixtureCausalBlock(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        # print(out.shape)#torch.Size([32, 512, 192])
        ...
        print(out_t.shape)#torch.Size([32, 512, 192])


        out = torch.cat([out, out_t], dim=1)
        print(out.shape)#torch.Size([32, 1024, 192])
        out = out.chunk(2)
        # print(out.shape)
        out = torch.cat([out[0], out[1].flip([-1])], dim=1)
        print(out.shape)#torch.Size([16, 2048, 192])
        out=rearrange(out, "b d l -> b l d")
        print(out.shape)#torch.Size([16, 192, 2048])
