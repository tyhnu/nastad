...
from opentad.cores import train_one_epoch_supernet, val_one_epoch_supernet, eval_one_epoch_supernet, build_optimizer, build_scheduler
    ...
    for epoch in range(resume_epoch + 1, max_epoch):
        ...
        if epoch >= val_start_epoch:
            if (cfg.workflow.val_loss_interval > 0) and ((epoch + 1) % cfg.workflow.val_loss_interval == 0):
                val_loss = val_one_epoch_supernet(
                    val_loader,
                    model,
                    logger,
                    args.rank,
